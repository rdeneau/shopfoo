...
type Filters
    ...
    member val Model = filters

    member _.apply products provider =
        let getSortKey (i, product) = [
            match filters.SortBy with
            | Some(ProductSort.Num, _) -> SortKeyPart.Num i
            | Some(ProductSort.Title, _) -> SortKeyPart.Text product.Title
            ...
            | Some(ProductSort.BookAuthors, _) ->
                ...
                SortKeyPart.Text product.Title

            | Some(ProductSort.StoreCategory, _) ->
                match product.Category with
                | Category.Store { Category = StoreCategory.Clothing } -> SortKeyPart.Text translations.Product.StoreCategory.Clothing
                | Category.Store { Category = StoreCategory.Electronics } -> SortKeyPart.Text translations.Product.StoreCategory.Electronics
                | Category.Store { Category = StoreCategory.Jewelry } -> SortKeyPart.Text translations.Product.StoreCategory.Jewelry
                | _ -> ()
    ...
    member _.highlight (fullText: string) (key: string) = [
    ...
    member _.tabs (products: Product list) (selectedProvider: Provider option) selectProvider =
...
and private Tab(filters: FiltersModel, translations: AppTranslations) =
    member _.authors products : ReactElement =
        let authors =
            products
            |> List.collect (fun p ->
    ...
    member _.storeCategory storeCategory text iconifyIcon page : ReactElement =
    ...
    member _.search provider : ReactElement =
